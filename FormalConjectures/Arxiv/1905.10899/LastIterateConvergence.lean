...
noncomputable def coefficients_schedule_p
  (p gamma K : ℝ)
  (t : ℕ) : ℝ × ℝ :=
  (1 / (K * Real.rpow ((t : ℝ) + gamma) p), gamma / ((t : ℝ) + gamma))

/--
Divergence conjecture for $0 < p < 1/2$.

The paper states that for $p < 1/2$, the discrete method simulates the ODE poorly and 
simple tests on bilinear examples show divergence. This open problem asks to formalize 
the divergence for a class of problems when $p \in (0, 1/2)$.
-/
...
theorem divergence_for_p_lt_half
  ...
  (z_init : Z_space n m)
  (h_init : s_seq 0 = { z := z_init, z0 := z_init, t := 0, history := [z_init]})
  (h_step : ∀ t, s_seq (t + 1) = step n m L (s_seq t) (coefficients_schedule_p p gamma K t).1 (coefficients_schedule_p p gamma K t).2)
  ...
  -- Negation of Convergence Result: it does not converge at the rate C/T or diverges
  ¬ (∃ C : ℝ, 0 < C ∧ ∀ T : ℕ, 1 ≤ T →
    let s_T := s_seq T
    ‖F_operator n m L s_T.z‖ ^ 2 ≤ C / (T : ℝ)) := by
  sorry
